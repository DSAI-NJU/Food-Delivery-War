...
posts = pd.read_csv('data/data/xhs/csv/处理/posts_processed.csv', encoding='utf-8')
comments = pd.read_csv('data/data/xhs/csv/处理/comments_processed.csv', encoding='utf-8')
...
RUNTIME_OUTPUT_DIR = Path('data/data/xhs/csv/处理')
...
# 优先读取现有 CSV；若不存在则使用默认事件列表
events_csv = RUNTIME_OUTPUT_DIR / '关键事件时间线.csv'
if events_csv.exists():
    key_events = pd.read_csv(events_csv, encoding='utf-8')
...
# ===== 2.3.a 月度均分图（评论/帖子比率） =====
ratio = (comment_counts_aligned / post_counts_aligned.replace(0, np.nan)).fillna(0)
fig_ratio, ax_ratio = plt.subplots(figsize=(14, 6))
ax_ratio.bar(range(len(ratio)), ratio.values, color='#8EC5FF', edgecolor='white', linewidth=1.5)
ax_ratio.set_xticks(range(len(ratio)))
ax_ratio.set_xticklabels(post_counts_str, rotation=45, ha='right')
ax_ratio.set_xlabel('月份')
ax_ratio.set_ylabel('评论/帖子 均值')
ax_ratio.set_title('月度评论/帖子均值')
ax_ratio.yaxis.grid(True, alpha=0.25, linestyle='--')
plt.tight_layout()
ratio_path = RUNTIME_OUTPUT_DIR / '月度评论帖子均值.png'
plt.savefig(ratio_path, dpi=300, bbox_inches='tight', facecolor='white')
plt.close(fig_ratio)
print(f"✓ 月度均分图已保存：{ratio_path}")
...
# 保存图表
chart_path = RUNTIME_OUTPUT_DIR / '关注度随时间变化.png'
plt.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"✓ 图表已保存：{chart_path}\n")

# ===== 2.3.b 日度30天滚动均值（帖子+评论） =====
daily_posts = posts.set_index('time').resample('D').size().rename('posts')
daily_comments = comments.set_index('create_time').resample('D').size().rename('comments')
daily = pd.concat([daily_posts, daily_comments], axis=1).fillna(0)
daily['total'] = daily['posts'] + daily['comments']
rolling = daily.rolling(30, min_periods=1).mean()

fig_roll, ax_roll = plt.subplots(figsize=(14, 6))
ax_roll.plot(daily.index, daily['total'], color='#99A8B2', alpha=0.4, label='日总量')
ax_roll.plot(rolling.index, rolling['total'], color='#8EC5FF', linewidth=2.5, label='30天滚动均值')
ax_roll.set_title('关注度日度与30天滚动均值', fontweight='bold')
ax_roll.set_xlabel('日期')
ax_roll.set_ylabel('总量（帖子+评论）')
ax_roll.legend()
ax_roll.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
plt.xticks(rotation=45, ha='right')
ax_roll.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()
roll_path = RUNTIME_OUTPUT_DIR / '关注度30天滚动均值.png'
plt.savefig(roll_path, dpi=300, bbox_inches='tight', facecolor='white')
plt.close(fig_roll)
print(f"✓ 30天滚动均值图已保存：{roll_path}\n")
...
weekly_chart_path = RUNTIME_OUTPUT_DIR / '关注度周度分析.png'
